# File 02 Migration Guide — 1.2.6

## Migration model

File 02 migration remains additive, idempotent and non-destructive. The 1.2.6 candidate advances File 02 DB identity to `1.2.1` and passkey schema identity to `1.0.1`; migration readiness now proves required columns and security-critical indexes, and legacy passkey credential columns are reconciled before successful markers are published.
...
1. Fresh installation of 1.2.6.
2. Upgrade from every repository-supported File 02 release to 1.2.6.
3. Upgrade from legacy `sa_*` tables/options/pages to canonical `sauth_*` storage and names.
4. Upgrade from 1.1.0/1.2.0 to 1.2.6 with additive passkey table/page creation and no password/Google/session data loss.
...
1. Install the exact deterministic 1.2.6 package only after its release build is separately produced and approved; registration/provider/passkey mutations remain gated.
2. Activate or run guarded File 02 repair.
3. Confirm `sauth_version` is `1.2.6`, `sauth_db_version` is `1.2.1`, and `sauth_passkey_schema_version` is `1.0.1`.
