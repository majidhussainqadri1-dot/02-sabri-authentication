...
# 1. Product correction: reconcile a MariaDB-preserved canonical key name that is bound to the legacy hash column.
...
"""
if old not in s:
    raise SystemExit('R335 prepare_legacy preimage missing')
	...
	private static function reconcile_legacy_credential_hash_index( $table ) {
		...
		foreach ( $rows as $row ) {
			...
			if ( ! isset( $indexes[ $name ] ) ) {
				$indexes[ $name ] = array( 'non_unique' => (int) ( $row['Non_unique'] ?? 1 ), 'columns' => array() );
			}
		...
		if ( 0 !== (int) $current['non_unique'] || array( 'credential_hash' ) !== $current['columns'] ) {
			return false;
		}

		$legacy_unique_exists = false;
		foreach ( $indexes as $name => $index ) {
			if ( 'credential_lookup_hash' !== $name && 0 === (int) $index['non_unique'] && array( 'credential_hash' ) === $index['columns'] ) {
				$legacy_unique_exists = true;
				break;
			}
		}
		$legacy_name = 'credential_hash_legacy';
		if ( isset( $indexes[ $legacy_name ] ) && ( 0 !== (int) $indexes[ $legacy_name ]['non_unique'] || array( 'credential_hash' ) !== $indexes[ $legacy_name ]['columns'] ) ) {
			return false;
		}
		...
		foreach ( $verify as $row ) {
			$name = (string) ( $row['Key_name'] ?? '' );
			if ( 'credential_lookup_hash' === $name ) { $canonical_name_free = false; }
...
'''
if anchor not in s:
    raise SystemExit('R335 helper insertion anchor missing')
s = s.replace(anchor, helper + anchor, 1)
p.write_text(s)

# 2. Current runtime/release identity -> 1.2.6. Historical review-evidence remains immutable.
current_top = [
...
for base in (Path('tests'), Path('.github/workflows')):
    for p in base.rglob('*'):
        if not p.is_file() or p.suffix not in {'.php','.yml','.yaml'}: continue
        # The R335 diagnostic is temporary and removed below; no need to preserve its current identity.
        p.write_text(current_sync(p.read_text()))

# 3. WordPress readme: current metadata/prose, new history, retain 1.2.5 history.
p = Path('readme.txt'); s = p.read_text()
if 'Stable tag: 1.2.5' not in s: raise SystemExit('R335 readme stable-tag preimage missing')
s = s.replace('Stable tag: 1.2.5', 'Stable tag: 1.2.6', 1)
...
# 4. Markdown changelog new current entry.
p = Path('CHANGELOG.md'); s = p.read_text()
anchor = '## 1.2.5 — Passkey dbDelta Migration Compatibility Candidate'
...
# 5. Release lock stays blocked until exact full integration passes.
p = Path('RELEASE-LOCK.json'); data = json.loads(p.read_text())
data['release_version'] = new_version
data['candidate_branch'] = new_branch
data['package_name'] = '02-sabri-authentication-1.2.6-SOURCE-CANDIDATE.zip'
data['manifest_name'] = '02-sabri-authentication-1.2.6-MANIFEST.json'
data['status']['coded'] = 'passkey_index_reconciliation_candidate_r335_corrected'
data['status']['packaged'] = 'not_claimed_from_review_branch'
data['status']['automated_qa'] = 'exact_head_pending_after_r335'
data['status']['staging_accepted'] = False
data['status']['live_deployed'] = False
data['status']['operational'] = False
data['review_line'] = 'R331-R335-corrective'
data['cross_file_blockers'] = [
    'Exact File 00 1.2.44 / File 02 1.2.6 WordPress-MariaDB integration must pass the full deferred-bootstrap, taxonomy, legacy passkey-index/column upgrade and logical-identity migration sequence before cross-file release closure.'
]
p.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n')

# 6. Permanent regression for exact R335 state.
...
# 7. Current exact integration follows the new runtime identity.
p = Path('.github/workflows/file00-1.2.44-real-integration.yml')
p.write_text(current_sync(p.read_text()))

# 8. Temporary diagnostic/correction machinery may not survive corrected source.
for temp in (
    Path('.github/workflows/r335-passkey-index-diagnostic.yml'),
    Path('.github/workflows/tmp-r335-passkey-index-reconciliation.yml'),
    Path('tools/apply-r335-passkey-index-reconciliation.py'),
):
