...
$req( false !== strpos( $main, 'Version: 1.3.1' ) && false !== strpos( $main, "SAUTH_VERSION', '1.3.1" ), 'runtime identity not 1.3.1' );
$req( false !== strpos( $main, "SAUTH_DB_VERSION', '1.3.0" ), 'DB identity not 1.3.0' );
$req( is_array( $lock ) && '1.3.1' === ( $lock['release_version'] ?? '' ), 'release lock not 1.3.1' );
...
$req( false !== strpos( $readme, 'Stable tag: 1.3.1' ) && false !== strpos( $readme, '= 1.3.1 =' ) && false !== strpos( $readme, '= 1.3.0 =' ) && false !== strpos( $readme, '= 1.2.3 =' ), 'WordPress readme identity/history invalid' );
$req( false !== strpos( $status, 'Version 1.3.0' ), 'status no longer preserves R337 base identity' );
$req( false !== strpos( $baseline, "RELEASE_VERSION: '1.3.1'" ), 'baseline workflow identity stale' );
$req( false !== strpos( $docs, "'release_version':'1.3.1'" ), 'docs workflow identity stale' );
$req( false !== strpos( $integration, "FILE02_VERSION: '1.3.1'" ) && false !== strpos( $integration, "FILE00_VERSION: '1.2.44'" ), 'real integration paired identity stale' );
...
echo 'R332 release identity invariants PASS (15 assertions).' . PHP_EOL;
