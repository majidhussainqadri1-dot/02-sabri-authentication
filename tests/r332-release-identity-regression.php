...
$req( is_array( $lock ) && '1.3.3' === ( $lock['release_version'] ?? '' ), 'release lock not 1.3.3' );
$req( '1.3.0' === ( $lock['database_version'] ?? '' ) && '1.0.1' === ( $lock['passkey_schema_version'] ?? '' ), 'schema identities stale' );
$req( 'fix/file02-passkey-assurance-cycle-1.3.3-2026-08-19' === ( $lock['candidate_branch'] ?? '' ), 'candidate branch identity stale' );
$req( 'R340-passkey-assurance-cycle' === ( $lock['review_line'] ?? '' ), 'current review line identity stale' );
...
$req( false !== strpos( $readme, 'Stable tag: 1.3.3' ) && false !== strpos( $readme, '= 1.3.3 =' ) && false !== strpos( $readme, '= 1.3.2 =' ) && false !== strpos( $readme, '= 1.3.1 =' ) && false !== strpos( $readme, '= 1.3.0 =' ) && false !== strpos( $readme, '= 1.2.3 =' ), 'WordPress readme identity/history invalid' );
$req( false !== strpos( $status, 'Version 1.3.0' ), 'status no longer preserves R337 base identity' );
$req( false !== strpos( $baseline, "RELEASE_VERSION: '1.3.3'" ), 'baseline workflow identity stale' );
$req( false !== strpos( $docs, "'release_version':'1.3.3'" ), 'docs workflow identity stale' );
$req( false !== strpos( $integration, "FILE02_VERSION: '1.3.3'" ) && false !== strpos( $integration, "FILE00_VERSION: '1.2.44'" ), 'real integration paired identity stale' );
...
echo 'R332 release identity invariants PASS (23 assertions).' . PHP_EOL;
