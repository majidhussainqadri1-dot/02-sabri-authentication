...
$req( 'repository_integration_green' === (string) ( $evidence['status'] ?? '' ), 'historical cross-file integration status is not green' );
$req( 'historical_pre_r337' === (string) ( $evidence['evidence_scope'] ?? '' ), 'R336 integration evidence is not scoped to its exact historical inputs' );
$req( true === ( $evidence['current_r337_revalidation_required'] ?? false ), 'current R337 paired revalidation is not required' );
$req( 31850253635 === (int) ( $evidence['workflow_run_id'] ?? 0 ), 'cross-file integration run ID is not the proven historical run' );
$req( 'f740ca65fc33031b98d7d75e5f27b7ccbeeefbf9' === (string) ( $evidence['file02_sha'] ?? '' ), 'proven historical File 02 integration SHA is stale' );
...
$req( false !== strpos( $status, '31850253635' ) && false !== strpos( $status, 'historical pre-R337' ) && false !== strpos( $status, 'post-R337' ), 'status does not preserve historical integration closure while requiring current revalidation' );
$req( false !== strpos( $manifest, '31850253635' ) && false !== strpos( $manifest, 'historical pre-R337' ) && false !== strpos( $manifest, 'post-R337' ), 'release manifest does not preserve historical integration closure while requiring current revalidation' );
...
echo 'R336 post-integration release closure regression PASS (26 assertions).' . PHP_EOL;
