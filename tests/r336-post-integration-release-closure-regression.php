...
$req( '1.3.0' === (string) ( $lock['release_version'] ?? '' ) && '1.3.0' === (string) ( $lock['database_version'] ?? '' ) && '1.0.1' === (string) ( $lock['passkey_schema_version'] ?? '' ), 'current release/schema identity is stale' );
$req( ! empty( $lock['cross_file_blockers'] ?? array() ), 'current File 02 1.3.0 integration blocker was falsely closed' );
...
$req( 'pending_exact_head' === (string) ( $current['status'] ?? '' ), 'current cross-file integration status is not pending exact head' );
...
$req( false !== strpos( $status, 'Historical paired evidence' ) && false !== strpos( $status, 'integration gate is **open**' ), 'status does not separate historical and current integration truth' );
$req( false !== strpos( $manifest, 'Historical run `31850253635`' ) && false !== strpos( $manifest, 'integration is pending' ), 'release manifest does not separate historical and current integration truth' );
