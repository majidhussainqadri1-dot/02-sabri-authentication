...
$req( '738aa2ab4b8be3d6cfdb3ecd46c88aa8d9ece3a0' === (string) ( $current['file00_sha'] ?? '' ) && '1d7f215193d778b0977c8e50d738c42e1e5f66c2' === (string) ( $prior['file00_sha'] ?? '' ), 'File 00 current/historical exact pins are stale' );
...
$req( false === ( $lock['live_1_3_0_activation_incident']['live_resolution_claimed'] ?? true ), 'historical R338 incident record unexpectedly changed without its own evidence update' );
$req( false === ( $lock['live_1_3_1_membership_route_contract_incident']['live_resolution_claimed'] ?? true ), 'historical R339 incident record unexpectedly changed without its own evidence update' );
$req( false === ( $lock['live_1_3_1_membership_route_contract_incident']['exact_deployed_file02_source_parity_verified'] ?? true ), 'historical R339 exact deployed source-parity field changed without evidence' );
$req( true === ( $lock['live_1_3_2_passkey_assurance_cycle_incident']['live_resolution_claimed'] ?? false ), 'R340 live-verified resolution truth was lost' );
$req( 'live_verified_resolved' === (string) ( $lock['live_1_3_2_passkey_assurance_cycle_incident']['status'] ?? '' ), 'R340 live-verified resolution status was lost' );
...
$req( false === ( $lock['live_1_3_3_legacy_email_verification_reconciliation_incident']['live_resolution_claimed'] ?? true ), 'R341 incident was falsely marked live-resolved before deployment/retest' );
...
foreach ( array(
	...
	'Prove R341 legacy verified-email reconciliation against current File 00',
...
$req( file_exists( $root . '/tests/r341-email-verification-legacy-reconciliation-regression.php' ), 'R341 reconciliation regression is missing' );
