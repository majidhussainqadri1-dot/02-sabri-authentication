...
$fail = array();
$checks = array(
    array( $main, 'Version: 1.2.3', 'plugin header release identity stale' ),
    array( $main, "SAUTH_VERSION', '1.2.3", 'runtime release identity stale' ),
    array( $main, "SAUTH_DB_VERSION', '1.2.1", 'DB identity unexpectedly changed' ),
    array( $lock, '"release_version": "1.2.3"', 'release lock runtime stale' ),
    array( $lock, 'review/file02-r321-r330-2026-08-14', 'release lock branch stale' ),
    array( $readme, 'Stable tag: 1.2.3', 'WordPress stable tag stale' ),
    array( $status, 'Version 1.2.3', 'status runtime stale' ),
    array( $manifest, '1.2.3 / 1.2.1', 'release manifest identity stale' ),
    array( $baseline, "RELEASE_VERSION: '1.2.3'", 'baseline release workflow version stale' ),
