...
$fail = array();
$checks = array(
    array( $main, 'Version: 1.3.0', 'plugin header release identity stale' ),
    array( $main, "SAUTH_VERSION', '1.3.0", 'runtime release identity stale' ),
    array( $main, "SAUTH_DB_VERSION', '1.3.0", 'DB identity unexpectedly changed' ),
    array( $lock, '"release_version": "1.3.0"', 'release lock runtime stale' ),
    array( $lock, 'agent/file02-comprehensive-remediation-1.3.0', 'release lock branch stale' ),
    array( $readme, 'Stable tag: 1.3.0', 'WordPress stable tag stale' ),
    array( $status, 'Version 1.3.0', 'status runtime stale' ),
    array( $manifest, '1.3.0 / 1.3.0', 'release manifest identity stale' ),
    array( $baseline, "RELEASE_VERSION: '1.3.0'", 'baseline release workflow version stale' ),
