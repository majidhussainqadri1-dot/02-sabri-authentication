...
$fail = array();
$checks = array(
    array( $main, 'Version: 1.3.1', 'plugin header release identity stale' ),
    array( $main, "SAUTH_VERSION', '1.3.1", 'runtime release identity stale' ),
    array( $main, "SAUTH_DB_VERSION', '1.3.0", 'DB identity unexpectedly changed' ),
    array( $lock, '"release_version": "1.3.1"', 'release lock runtime stale' ),
    array( $lock, 'fix/file02-live-passkey-user-status-index-1.3.1', 'release lock branch stale' ),
    array( $readme, 'Stable tag: 1.3.1', 'WordPress stable tag stale' ),
    array( $status, 'Version 1.3.0', 'status no longer preserves the 1.3.0 R337 base history' ),
    array( $manifest, '1.3.0 / 1.3.0', 'release manifest no longer preserves the R337 base identity' ),
    array( $baseline, "RELEASE_VERSION: '1.3.1'", 'baseline release workflow version stale' ),
