...
$fail = array();
$checks = array(
    array( $main, 'Version: 1.3.2', 'plugin header release identity stale' ),
    array( $main, "SAUTH_VERSION', '1.3.2", 'runtime release identity stale' ),
    array( $main, "SAUTH_DB_VERSION', '1.3.0", 'DB identity unexpectedly changed' ),
    array( $lock, '"release_version": "1.3.2"', 'release lock runtime stale' ),
    array( $lock, 'fix/file02-file00-canonical-routes-1.3.2-2026-08-16', 'release lock branch stale' ),
    array( $lock, 'R339-file00-canonical-route-contract', 'release lock review line stale' ),
    array( $readme, 'Stable tag: 1.3.2', 'WordPress stable tag stale' ),
    array( $readme, '= 1.3.2 =', 'WordPress changelog omits current route-contract hotfix' ),
    ...
    array( $baseline, "RELEASE_VERSION: '1.3.2'", 'baseline release workflow version stale' ),
