...
$req( false !== strpos( $main, 'Version: 1.3.2' ) && false !== strpos( $main, "SAUTH_VERSION', '1.3.2" ), 'current runtime release identity is not synchronized' );
...
$req( $coded_round >= 339, 'release lock coded status regressed below the R339 corrective baseline' );
$req( $review_round >= 339, 'release lock review line regressed below the R339 corrective baseline' );
...
$req( false !== strpos( file_get_contents( $root . '/readme.txt' ), 'Stable tag: 1.3.2' ), 'current WordPress release identity is not 1.3.2' );
$req( false !== strpos( file_get_contents( $root . '/readme.txt' ), '= 1.3.2 =' ), 'WordPress changelog does not record 1.3.2' );
...
echo 'R330 final adversarial invariants PASS (21 assertions).' . PHP_EOL;
