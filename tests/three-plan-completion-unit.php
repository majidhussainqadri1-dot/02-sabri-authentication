 ...
 * present in File 02 v1.2.3 while the fourth-plan passkey extension, the
...
sauth_three_plan_require( $readme, array( 'Stable tag: 1.2.3', '/account/sessions/', 'Google-first registration', 'Passkey', 'city', 'ethical' ), 'readme' );

$status = sauth_three_plan_read( $root, 'STATUS.md' );
sauth_three_plan_require( $status, array( 'Version 1.2.3', 'Source coding', 'Automated-QA', 'Staging-Accepted', 'Operational', 'Passkey' ), 'status truth' );

$workflow = sauth_three_plan_read( $root, '.github/workflows/baseline-integrity.yml' );
sauth_three_plan_require( $workflow, array( 'three-plan-completion-unit.php', 'passkey-webauthn-unit.php', 'tests/r32*-regression.php', 'deterministic-package', 'upload-artifact', '1.2.3' ), 'release workflow' );

echo "File 02 prior three-plan requirements preserved inside 1.2.3 R321-R329 hardened candidate.\n";
