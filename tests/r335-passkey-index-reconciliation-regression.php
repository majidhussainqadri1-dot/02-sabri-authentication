...
$req( empty( $lock['cross_file_blockers'] ) && 'repository_integration_green' === (string) ( $integration_evidence['status'] ?? '' ), 'R335 historical blocker closure lacks green exact-input integration evidence' );
$req( 'historical_pre_r337' === (string) ( $integration_evidence['evidence_scope'] ?? '' ), 'R335 paired evidence is not scoped to its pre-R337 inputs' );
$req( true === ( $integration_evidence['current_r337_revalidation_required'] ?? false ), 'current R337 paired revalidation is not required' );
$req( 31850253635 === (int) ( $integration_evidence['workflow_run_id'] ?? 0 ), 'R335 closure points at the wrong historical integration run' );
