...
$req( is_array( $lock ) && '1.3.4' === ( $lock['release_version'] ?? '' ), 'release lock runtime stale' );
$req( 'fix/file02-legacy-email-verification-reconciliation-1.3.4' === ( $lock['candidate_branch'] ?? '' ), 'release lock branch stale' );
...
$req( '1.3.4' === (string) ( $integration_evidence['file02_runtime'] ?? '' ), 'current exact-head integration runtime identity is stale' );
$req( '738aa2ab4b8be3d6cfdb3ecd46c88aa8d9ece3a0' === (string) ( $integration_evidence['file00_sha'] ?? '' ), 'current exact-head File 00 integration identity is stale' );
...
$req( false !== strpos( $integration, "FILE00_VERSION: '1.2.44'" ) && false !== strpos( $integration, "FILE02_VERSION: '1.3.4'" ), 'paired integration identities stale' );
$req( false !== strpos( $integration, '738aa2ab4b8be3d6cfdb3ecd46c88aa8d9ece3a0' ), 'File 00 exact integration pin stale' );
$req( false !== strpos( $integration, 'Prove R341 legacy verified-email reconciliation against current File 00' ), 'R341 email reconciliation proof missing from exact integration' );
...
echo 'R334 passkey dbDelta migration invariants PASS (25 assertions).' . PHP_EOL;
